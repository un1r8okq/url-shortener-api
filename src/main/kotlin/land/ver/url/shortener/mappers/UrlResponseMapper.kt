...
class UrlResponseMapper {
    fun map(url: Url) = UrlResponse(
        ...
        lastVisitTimestampUtc = getLatestVisit(url),
    )

    private fun getLatestVisit(url: Url): String? {
        val latestVisit = url.urlVisits.maxByOrNull { it.timestampUtc }

        return latestVisit?.timestampUtc?.toString()
    }
}
